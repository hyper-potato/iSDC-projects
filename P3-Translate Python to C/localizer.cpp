...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  vector <float> this_row;
  int height = grid.size();
  int width = grid[0].size();
  int area = height * width;
  float each_belief = 1.0 / area;

  for (int row = 0; row<height; row++){
    this_row.clear();
    
    for (int col = 0; col<width; col++){
      this_row.push_back(each_belief);
    }
    initial_grid.push_back(this_row);
  }
...
vector< vector <float> > move(int dy, int dx,
    ...
    for (int row = 0; row < height; row++){
        for (int col = 0 ; col < width; col++){
            new_row = fmod((row + dy), height);
            new_col = fmod((col + dx), width);
            newGrid[int(new_row)][int(new_col)] = beliefs[row][col];
        }
    }
