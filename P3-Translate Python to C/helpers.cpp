...
vector<vector<float> > normalize(vector<vector<float> > grid) {
    ...
    vector<float> row;
    // Get the sum of values in the grid
    float total = 0;
    ...
    // Divide each cell by the sum of all values
    for (int i = 0; i < height; i++) {
        row.clear();
        for (int j = 0; j < width; j++) {
...
vector<vector<float> > blur(vector<vector<float> > grid, float blurring) {
    ...
    float diagonal_prob = blurring / 12.0;
    float adjacent_prob = blurring / 6.0;

    vector < vector <float> >  window {
            {diagonal_prob, adjacent_prob, diagonal_prob },
...
//                    if (i + dy < 0) {
//                        new_i = (height - (dy - i)) % height;
//                    } else {
//                        new_i = (i + dy) % height;
//                    }
//
//                    if (j + dx < 0) {
//                        new_j = (width - (dx - j)) % width;
//                    } else {
//                        new_j = (j + dx) % width;
//                    }

                    newGrid[new_i][new_j] += mult * cell;
